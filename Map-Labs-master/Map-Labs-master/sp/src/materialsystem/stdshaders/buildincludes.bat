...
REM REM ****************
REM REM Add the executables to the worklist.
REM REM ****************
REM if /i "%DIRECTX_SDK_VER%" == "pc09.00" (
	REM rem echo "Copy extra files for dx 9 std
REM )
REM if /i "%DIRECTX_SDK_VER%" == "pc09.30" (
	REM echo %SrcDirBase%\devtools\bin\d3dx9_33.dll >> filestocopy.txt
REM )

REM echo %SrcDirBase%\%DIRECTX_SDK_BIN_DIR%\dx_proxy.dll >> filestocopy.txt

REM echo %SDKBINDIR%\shadercompile.exe >> filestocopy.txt
REM echo %SDKBINDIR%\shadercompile_dll.dll >> filestocopy.txt
REM echo %SDKBINDIR%\vstdlib.dll >> filestocopy.txt
REM echo %SDKBINDIR%\tier0.dll >> filestocopy.txt

REM REM ****************
REM REM Cull duplicate entries in work/build list
REM REM ****************
REM if exist filestocopy.txt type filestocopy.txt | perl "%SrcDirBase%\devtools\bin\uniqifylist.pl" > uniquefilestocopy.txt
REM if exist filelistgen.txt if not "%dynamic_shaders%" == "1" (
    REM echo Generating action list...
    REM copy filelistgen.txt filelist.txt >nul
REM )

REM REM ****************
REM REM Execute distributed process on work/build list
REM REM ****************

REM set shader_path_cd=%cd%
REM if exist "filelist.txt" if exist "uniquefilestocopy.txt" if not "%dynamic_shaders%" == "1" (
	...
	REM echo %shadercompilecommand% %SDKArgs% -shaderpath "%shader_path_cd:/=\%" -allowdebug
	REM %shadercompilecommand% %SDKArgs% -shaderpath "%shader_path_cd:/=\%" -allowdebug
	REM cd /D %shader_path_cd%
REM )

REM REM ****************
REM REM PC Shader copy
REM REM Publish the generated files to the output dir using XCOPY
REM REM This batch file may have been invoked standalone or slaved (master does final smart mirror copy)
REM REM ****************
REM :DoXCopy
REM if not "%dynamic_shaders%" == "1" (
REM if not exist "%targetdir%" md "%targetdir%"
