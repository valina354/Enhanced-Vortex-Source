...
//
// Purpose: Sensory dummy which doesn't do anything except sense.
// 
...
// For example, a model-less dummy could be parented to the player and use their relationships, positions, etc.
...
void CAI_SensingDummy<BASE_NPC>::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		// The issue is that sensing dummies can't turn their bodies, so any orders to "face" end up stuck on the face tasks.
