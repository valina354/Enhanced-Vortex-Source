...
#else
#ifdef MAPBASE
ConVar r_flashlightdepthres( "r_flashlightdepthres", "5596" );
#else
...
bool CClientShadowMgr::Init()
{
	...
	m_nMaxDepthTextureShadows = bTools ? 4 : 101;	// Just one shadow depth texture in games, more in tools
#else
	// 101 lets mappers use up to 100 shadow-casting projected textures, which is better than 100.
	int iNumShadows = CommandLine()->ParmValue( "-numshadowtextures", 101 );
	...
	mat_slopescaledepthbias_shadowmap.SetValue( "1" ); // Would do something like 2 here, but it causes citizens to look weird under flashlights
